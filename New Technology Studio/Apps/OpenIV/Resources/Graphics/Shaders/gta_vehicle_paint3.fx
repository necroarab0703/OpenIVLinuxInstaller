...
VS_OUTPUT VSBasic( VS_INPUT i )
{
	...
	Output.TextureUV2 = i.vTexCoord1;
    return Output;
}
...
{ 
	...
	if (gTextured < 0.99) {
		...
		outCol.a = 1.0f;
		float4 r4 = tex2D( DiffuseTex, In.TextureUV );// * float4(0.96, 0.53, 0.058, 1.0); //taxi color
		float4 r1 = tex2D( DiffuseTex2, In.TextureUV2 );
